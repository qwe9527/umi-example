...
interface IDocCookies {
  ...
  setItem: (sKey: string, sValue: string, vEnd?: any, sPath?: any, sDomain?: string, bSecure?: string) => boolean,
  removeItem: (sKey: string, sPath?: string, sDomain?: string) => boolean,
  hasItem: (sKey: string) => boolean,
  keys: () => string[]
}

const docCookies: IDocCookies = {
  getItem: function (sKey: string) {
    return decodeURIComponent(document.cookie.replace(new RegExp("(?:(?:^|.*;)\\s*" + encodeURIComponent(sKey).replace(/[\-\.\+\*]/g, "\\$&") + "\\s*\\=\\s*([^;]*).*$)|^.*$"), "$1")) || null;
  },
  setItem: function (sKey: string, sValue: string, vEnd?: any, sPath?: any, sDomain?: string, bSecure?: string) {
